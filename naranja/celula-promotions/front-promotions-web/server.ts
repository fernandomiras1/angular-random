...
const template = fs.readFileSync(path.join(process.cwd(), 'dist/promociones', 'index.html')).toString();
...
const DIST_FOLDER = join(process.cwd(), 'dist/promociones');
...
app.get('manifest.json', (req, res) => {
  res.sendFile(join(DIST, 'manifest.json'), {
    headers: {
      'Cache-Control': 'public, max-age=0',
    },
  });
