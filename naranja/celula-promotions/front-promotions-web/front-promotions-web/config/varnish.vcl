...
backend default {
  .host = "{{ default "front-promotions-web" .Values.varnish.varnishBackendService }}";
...
sub vcl_recv {
  ...
  if (req.http.Accept-Encoding) {
    if (req.url ~ "\.(jpg|jpeg|png|gif|gz|tgz|bz2|tbz|mp3|ogg|swf|woff|woff2|ttf|svg)$") {
  ...
  # ------------------------------------------------------------------------
  if (
    req.url ~ "^/promociones/sugeridas"
  ) {
    return (synth(301, "/promociones/todas"));
  }
