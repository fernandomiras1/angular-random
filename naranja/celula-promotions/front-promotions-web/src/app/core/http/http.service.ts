...
declare module '@angular/common/http/src/client' {
  export interface HttpClient {
    ...
    avoidCache(): HttpClient;

    addGeolocationCacheInterceptor(): HttpClient;

    removeConsumeInterceptor(): HttpClient;
  }
}
...
export class HttpService extends HttpClient {
  ...
  constructor(
    ...
    if (!this.interceptors) {
      this.interceptors = [
        ...
        this.injector.get(ServerStateInterceptor),
        this.injector.get(WatchdogInterceptor),
        this.injector.get(BrowserStateInterceptor),
        this.injector.get(ConsumerInterceptor),
  ...
  avoidCache(): HttpClient {
    const navigator = this.injector.get(NavigatorService);
    if (navigator.isServer) {
      return this.removeInterceptor(ServerStateInterceptor);
    }
    return this.removeInterceptor(BrowserStateInterceptor);
  }

  removeConsumeInterceptor(): HttpClient {
    return this.removeInterceptor(ConsumerInterceptor);
  }

  addGeolocationCacheInterceptor(): HttpService {
    return this.addInterceptor(this.injector.get(GeocodingCacheInterceptor));
  }
